...
int main(int argc, char **argv) {
  ...
  try {
    ...
    Kokkos::initialize( argc, argv );
    {
      ...
      Array2DR8::HostMirror h_A = createHostMirror( A );
      ...
      for ( int repeat = 0; repeat < nrepeat; repeat++ ) {
        ...
        // Output result.
        if ( repeat == ( nrepeat - 1 ) ) {
          std::cout << "  Computed result for " << N << " x " << M << " is "
               << result << std::endl;
        }
        ...
        std::cout << "  N( " << N << " ) M( " << M << " ) nrepeat ( " << nrepeat
             << " ) problem( " << Gbytes * 1000 << " MB ) time( " << time
             << " s ) bandwidth( " << Gbytes * nrepeat / time << " GB/s )"
             << std::endl;
