...
macro(common)
  ...
  option(OMEGA_COVERAGE "Instrument the build for code coverage and add the COVERAGE_REPORT ctest (default OFF)." OFF)

  # host/CPU line-coverage gate, in percent (decision 3; matches mam4xx codecov target).
  # COVERAGE_REPORT fails when measured host coverage is below this value. Tunable.
  set(OMEGA_COVERAGE_THRESHOLD "90" CACHE STRING "Host/CPU line-coverage gate (percent) for COVERAGE_REPORT")
...
# Inject code-coverage compile/link flags into OMEGA_CXX_FLAGS and
# OMEGA_LINK_OPTIONS, selecting the toolchain by compiler family (decision 2):
#   - GCC host / CPU baseline -> gcov/lcov via --coverage
#   - Clang-derived targets (SYCL icpx, clang CPU, HIP amdclang++, CUDA host-via-clang)
#     -> LLVM source-based coverage via -fprofile-instr-generate -fcoverage-mapping
# Must be called from init_standalone_build() AFTER OMEGA_ARCH and the C++ compiler
# are resolved but BEFORE OMEGA_CXX_FLAGS is folded into CMAKE_CXX_FLAGS.
# Sets OMEGA_COVERAGE_TOOLCHAIN to "gcov" or "llvm" for downstream consumers.
macro(set_coverage_flags)

  if(OMEGA_COVERAGE)

    # Coverage requires a Debug-style build to avoid optimized-build line skew
    # (mirrors EKAT's FATAL_ERROR). Omega's default build type is Release, so a
    # coverage build must explicitly request Debug.
    if(NOT ("${OMEGA_BUILD_TYPE}" STREQUAL "Debug" OR "${OMEGA_BUILD_TYPE}" STREQUAL "DEBUG"))
      message(FATAL_ERROR
        "OMEGA_COVERAGE=ON requires a Debug build to avoid optimized-build line "
        "skew. Re-run cmake with -DOMEGA_BUILD_TYPE=Debug "
        "(current OMEGA_BUILD_TYPE='${OMEGA_BUILD_TYPE}').")
    endif()

    # Detect compiler family. project() has not run yet, so CMAKE_CXX_COMPILER_ID
    # is unavailable here; probe the resolved compiler binary directly instead.
    # icpx/clang/amdclang++ (and nvcc host-via-clang) report a Clang-compatible
    # banner; plain gcc/g++ reports "gcc"/"Free Software Foundation".
    set(_OMEGA_COV_IS_CLANG "")
    if(OMEGA_CXX_COMPILER)
      execute_process(
        COMMAND ${OMEGA_CXX_COMPILER} --version
        RESULT_VARIABLE _OMEGA_COV_VER_RES
        OUTPUT_VARIABLE _OMEGA_COV_VER_OUT
        ERROR_QUIET)
      if(_OMEGA_COV_VER_RES EQUAL 0)
        string(REGEX MATCH "clang|Clang|DPC\\+\\+|oneAPI|IntelLLVM"
               _OMEGA_COV_IS_CLANG "${_OMEGA_COV_VER_OUT}")
      endif()
    endif()

    if(_OMEGA_COV_IS_CLANG OR "${OMEGA_ARCH}" STREQUAL "SYCL" OR
       "${OMEGA_ARCH}" STREQUAL "HIP")
      # LLVM source-based coverage (shared host+device toolchain on SYCL).
      set(OMEGA_COVERAGE_TOOLCHAIN "llvm")
      set(_OMEGA_COV_FLAGS "-fprofile-instr-generate -fcoverage-mapping")
    else()
      # gcc/gcov baseline.
      set(OMEGA_COVERAGE_TOOLCHAIN "gcov")
      set(_OMEGA_COV_FLAGS "--coverage")
    endif()

    message(STATUS "OMEGA_COVERAGE = ON (toolchain=${OMEGA_COVERAGE_TOOLCHAIN}, flags=${_OMEGA_COV_FLAGS})")

    # Compile flags go through OMEGA_CXX_FLAGS (folded into CMAKE_CXX_FLAGS just
    # below the call site). For CUDA the host channel needs -Xcompiler, but CUDA
    # is out of scope on this card (#2); the gcov/llvm host flags above cover the
    # in-scope SERIAL/OPENMP/SYCL paths.
    set(OMEGA_CXX_FLAGS "${OMEGA_CXX_FLAGS} ${_OMEGA_COV_FLAGS}")

    # Link flags go through OMEGA_LINK_OPTIONS, consumed by target_link_options on
    # the OmegaLibFlags INTERFACE target so every add_omega_test executable is
    # instrumented uniformly (decision: the active link path, not the commented-out
    # OMEGA_EXE_LINKER_FLAGS hook).
    separate_arguments(_OMEGA_COV_LINK_LIST NATIVE_COMMAND "${_OMEGA_COV_FLAGS}")
    list(APPEND OMEGA_LINK_OPTIONS ${_OMEGA_COV_LINK_LIST})
...
macro(init_standalone_build)
  ...
  # create a coverage helper script (decision 4: manual coverage runs +
  # optional Codecov upload; no GitHub Actions CI job). Generated only when
  # OMEGA_COVERAGE is ON so a normal build dir is unaffected.
  if(OMEGA_COVERAGE)
    set(_CoverageScript ${OMEGA_BUILD_DIR}/omega_coverage.sh)
    file(WRITE ${_CoverageScript}  "#!/usr/bin/env bash\n")
    file(APPEND ${_CoverageScript} "# Generated by OmegaBuild.cmake when -DOMEGA_COVERAGE=ON.\n")
    file(APPEND ${_CoverageScript} "# Runs the ctest suite (whose final COVERAGE_REPORT test\n")
    file(APPEND ${_CoverageScript} "# aggregates coverage and gates host coverage), then\n")
    file(APPEND ${_CoverageScript} "# optionally uploads to Codecov when CODECOV_TOKEN is set.\n\n")
    file(APPEND ${_CoverageScript} "set -e\n")
    file(APPEND ${_CoverageScript} "source ./omega_env.sh\n\n")
    ...
    file(APPEND ${_CoverageScript} "if [ -n \"\${CODECOV_TOKEN:-}\" ]; then\n")
    file(APPEND ${_CoverageScript} "  if command -v codecov >/dev/null 2>&1; then\n")
    file(APPEND ${_CoverageScript} "    codecov -f coverage.info -F omega_host || \\\n")
    file(APPEND ${_CoverageScript} "      echo 'WARN: codecov upload failed (non-fatal).'\n")
    file(APPEND ${_CoverageScript} "  else\n")
    file(APPEND ${_CoverageScript} "    echo 'WARN: CODECOV_TOKEN set but codecov CLI not found; skipping upload.'\n")
    file(APPEND ${_CoverageScript} "  fi\n")
    file(APPEND ${_CoverageScript} "else\n")
    file(APPEND ${_CoverageScript} "  echo 'NOTE: CODECOV_TOKEN unset; skipping Codecov upload (coverage.info kept locally).'\n")
    file(APPEND ${_CoverageScript} "fi\n")
    execute_process(COMMAND chmod +x ${_CoverageScript})
  endif()
...
#  endif()
  ...
  # inject coverage compile/link flags (OMEGA_COVERAGE) before folding
  # OMEGA_CXX_FLAGS into CMAKE_CXX_FLAGS; OMEGA_ARCH and the C++ compiler are
  # both resolved by this point.
  set_coverage_flags()
