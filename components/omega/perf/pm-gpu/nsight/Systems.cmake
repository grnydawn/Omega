...
set(_Script ${OMEGA_BUILD_DIR}/omega_profile_nsys.sh)
...
foreach(_SUBLIST IN LISTS _CTESTS)
    ...
    file(APPEND ${_Script} "cat << 'EOF' > test/${TEST_NAME}_nsys.sh\n")
    file(APPEND ${_Script} "#!/bin/bash\n")
    file(APPEND ${_Script} "if [[ \"$SLURM_PROCID\" -eq 0 ]]; then\n")
    file(APPEND ${_Script} "nsys profile -o ${TEST_NAME} --cuda-memory-usage=true --force-overwrite=true --trace=cuda,nvtx,osrt ./${EXE_NAME}\n")
    file(APPEND ${_Script} "else\n")
    file(APPEND ${_Script} "./${EXE_NAME}\n")
    file(APPEND ${_Script} "fi\n")
    file(APPEND ${_Script} "EOF\n")
    file(APPEND ${_Script} "chmod +x test/${TEST_NAME}_nsys.sh\n")
    file(APPEND ${_Script} "bash -c \"cd test; ${MPI_CMD} -- ./${TEST_NAME}_nsys.sh\"\n")
    file(APPEND ${_Script} "\n\n")
endforeach()
