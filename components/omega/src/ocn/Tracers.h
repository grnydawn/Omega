...
class Tracers {
 ...
 private:
   ...
   static std::vector<Array3DR8>
       TracerArrays; ///< TimeLevels -> [Tracer, Cell, Vert]
   static std::vector<HostArray3DR8>
   ...
   static int initParallelIO(I4 &CellDecompR8, Decomp *MeshDecomp);
   ...
   static std::vector<std::string> TracerDimNames; // Tracer dimension names
 ...
 public:
   ...
   static int getIndex(const std::string &TracerName, ///< [in] tracer name
                       int &TracerIndex               ///< [out] tracer index
   );

   static int getName(const int TracerIndex,  ///< [in] tracer index
                      std::string &TracerName ///< [out] tracer name
   );

   // returns all tracer device arrays. If it does not exist, return an empty
   // view
   static Array3DR8 getAll(const int TimeLevel ///< [in] time level index
   );

   // returns a tracer device array by tracer index. If it does not exist,
   // return an empty view
   static Array2DR8
   getByIndex(const int TimeLevel,  ///< [in] time level index
              const int TracerIndex ///< [in] global tracer index
   );

   // returns a tracer device array by tracer name. If it does not exist, return
   // an empty view
   static Array2DR8
   getByName(const int TimeLevel,          ///< [in] time level index
             const std::string &TracerName ///< [in] global tracer name
   );

   // returns all tracer device arrays. If it does not exist, return an empty
   // view
   static HostArray3DR8
   getAllHost(const int TimeLevel ///< [in] time level index
   );

   // returns a tracer host array by tracer index. If it does not exist, return
   // an empty view
   static HostArray2DR8
   getHostByIndex(const int TimeLevel,  ///< [in] time level index
                  const int TracerIndex ///< [in] global tracer index
   );

   // returns a tracer host array by tracer name. If it does not exist, return
   // an empty view
   static HostArray2DR8
   getHostByName(const int TimeLevel,          ///< [in] time level index
                 const std::string &TracerName ///< [in] global tracer name
   );

   // returns a field by tracer index. If it does not exist, return an empty
   // view
   static std::shared_ptr<Field>
   getFieldByIndex(const int TracerIndex ///< [in] global tracer index
   );

   // returns a field by tracer name. If it does not exist, return an empty view
   static std::shared_ptr<Field>
   ...
   static int
   getGroupRange(const std::string &GroupName,   ///< [in] group name
                 std::pair<int, int> &GroupRange ///< [out] group range
   );

   // check if a tracer is a member of group by tracer index
   static bool
   isGroupMemberByIndex(const int TracerIndex,      ///< [in] tracer index
   ...
   /// Exchange halo
   static int exchangeHalo(const int TimeLevel ///< [in] tracer time level
   );
   ...
   static int copyToDevice(const int TimeLevel ///< [in] tracer time level
   );

   /// Copy tracers variables from device to host
   static int copyToHost(const int TimeLevel ///< [in] tracer time level
   );
